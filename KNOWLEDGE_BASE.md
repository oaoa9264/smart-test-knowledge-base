...
**关键机制：**
- 项目可选绑定 `product_code`，用于声明“该项目归属哪个产品文档知识库”
...
### 4.15 产品文档 (`/api/product-docs`，P1+ 新增)
...
| **产品文档** | `/product-docs` | 导入 Markdown 产品文档、按章节查看/编辑分块、审核文档更新建议 |
...
4. 点击需求行 → 设为当前选中需求
5. 点击「查看」→ 打开需求详情弹窗，底部提供「导出规范化需求」按钮
6. 点击「导出规范化需求」后，前端先调用 `POST /api/requirements/{requirement_id}/normalized-doc-tasks` 发起后台任务
7. 若任务处于 `queued/running`，详情弹窗展示“后台生成中”提示；重新打开详情时会通过 `GET /normalized-doc-tasks/latest` 自动恢复状态
8. 若任务已完成，详情弹窗展示“上次已生成完成”提示；再次点击按钮会直接打开预览
9. 预览弹窗使用 `ReactMarkdown` 渲染模型返回的 Markdown，并根据 `uses_fresh_snapshot / snapshot_stale` 展示快照新鲜度提示
10. 点击「下载 Markdown」后，前端基于预览内容生成本地 `.md` 文件，不依赖额外导出接口
...
### 6.9 产品文档页面交互流程 (P1+ 新增)

**页面布局：**
- 左栏：产品文档列表，支持导入新文档、选择当前文档、删除文档
- 右栏：当前文档详情、分块列表、文档更新记录

**操作流程：**
1. 点击「导入文档」→ Modal 表单填写 `product_code / name / description / content(Markdown)`
2. 提交后调用 `POST /api/product-docs/import`，后端按 `##` / `###` 标题自动切分文档分块并建立关键词索引
3. 左侧文档列表展示产品名称、`product_code`、版本号 `vN`
4. 点击列表项 → 加载文档详情和更新记录；详情区展示产品标识、版本、描述、分块数
5. 分块列表使用 `Collapse` 展示，每个分块可单独进入编辑弹窗并保存内容
6. 更新记录表展示状态、分块 ID、风险项 ID；待审核记录支持查看原文/建议对比、应用更新、拒绝更新
7. 删除产品文档后会清空当前选中详情，并从左侧列表移除
8. 更新记录除本页手工维护外，也可能来自规则树页 RiskPanel 的“产品澄清 -> 需要更新产品文档”链路自动生成
9. 当前选中文档时可点击「手工补录知识」打开弹窗，填写补充事实和希望落库的澄清内容，前端会调用 `POST /api/product-docs/suggest-update` 生成待审核更新建议
