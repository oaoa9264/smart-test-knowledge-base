...
| **P1+** | 产品文档与产品知识沉淀 | 导入 Markdown 产品文档、按章节分块、关联项目，为风险分析提供产品背景知识 |
| **P1+** | 风险收敛与规范化需求导出 | 沉淀正式需求输入、生成带新鲜度标记的有效需求快照，并支持异步整理规范化需求 Markdown |
| **P1+** | 测试方案与用例生成 | AI 基于规则树生成测试方案 → 生成测试用例 → 确认导入用例库 |
| **P1+** | 追问分析与 PDF 草稿辅助 | 面向老项目/规则不完整场景，支持自由输入分析、PDF 拆解草稿、结构化追问结论与 Markdown 导出 |
...
创建项目 → 创建需求 → 构建规则树 → 创建测试用例(绑定规则节点)
                 ...
                 回归推荐 (FULL/CHANGE) → 输出最小回归集 + 解释
                    ...
                    │   风险收敛 + 规范化需求导出 (P1+ 新增)     │
                    │ 正式输入沉淀 → 有效需求快照 → 新鲜度校验    │
                    │ → 异步整理规范化需求 → 下载 Markdown       │
                    └────────────────────────────────────────┘

                    ┌────────────────────────────────────────┐
                    │      产品文档与产品知识 (P1+ 新增)        │
                    │ 导入 Markdown 产品文档 → 分块沉淀产品知识   │
                    │ → 项目绑定产品 → 风险分析召回背景知识       │
                    └────────────────────────────────────────┘
...
| pypdf | 6.10.0 | PDF 页数校验、全文提取 |
| pypdfium2 | 5.7.0 | PDF 页面渲染为图片，供多模态补充分析使用 |
...
│   │   │   ├── clarification_review.py # 追问分析记录接口 (P1+ 新增)
│   │   │   ├── clarification_review_pdf.py # 追问分析 PDF 草稿接口 (P1+ 新增)
│   │   │   ├── coverage.py         # 覆盖矩阵接口
│   │   │   ├── effective_requirements.py # 有效需求快照 / 同步分析兼容接口
│   │   │   ├── evidence_blocks.py  # 产品知识证据块接口
│   │   │   ├── normalized_requirement_doc_tasks.py # 规范化需求异步任务接口
│   │   │   ├── normalized_requirement_docs.py # 规范化需求预览接口
│   │   │   ├── product_docs.py     # 产品文档接口 (P1+ 新增)
│   │   │   ├── projects.py         # 项目 & 需求接口
│   │   │   ├── recommendation.py   # 回归推荐接口 (P1 新增)
│   │   │   ├── requirement_inputs.py # 需求输入补充接口
│   │   │   ├── risk_analysis_tasks.py # 风险收敛异步任务接口
...
│   │   │   └── schema_migrations.py # 数据库迁移辅助 (版本化/异步任务补列)
│   │   ├── models/
│   │   │   ├── __init__.py
...
│   │   │   ├── clarification_review.py # 追问分析 DTO (P1+ 新增)
│   │   │   ├── clarification_review_pdf.py # PDF 草稿 DTO (P1+ 新增)
│   │   │   ├── normalized_requirement_doc.py # 规范化需求预览 DTO
│   │   │   ├── normalized_requirement_doc_task.py # 规范化需求异步任务 DTO
│   │   │   ├── product_doc.py      # 产品文档 DTO (P1+ 新增)
│   │   │   ├── project.py          # 项目/需求 Pydantic DTO (含 Create/Update/Read)
│   │   │   ├── recommendation.py   # 回归推荐 DTO (P1 新增)
│   │   │   ├── risk.py             # 风险分析 DTO (P1+ 新增)
│   │   │   ├── risk_analysis_task.py # 风险收敛异步任务 DTO
│   │   │   ├── risk_convergence.py # 有效需求 / 证据块 / 分阶段分析 DTO
...
│   │   │   ├── clarification_review_service.py # 追问分析服务（含 v2 结果归一化）(P1+ 新增)
│   │   │   ├── cover_set.py        # 用例覆盖集合计算 (P1 新增)
│   │   │   ├── coverage.py         # 覆盖率矩阵计算
│   │   │   ├── fallback_llm_client.py # LLM 顺序回退编排器（配置驱动 provider chain）
│   │   │   ├── impact.py           # 变更影响分析
│   │   │   ├── impact_domain.py    # 变更影响域计算 (P1 新增)
│   │   │   ├── llm_client.py       # 向后兼容包装器（自动构建多模型回退链）
│   │   │   ├── llm_provider_factory.py # 配置驱动 provider 工厂
│   │   │   ├── llm_result_helpers.py # 统一 LLM 成功/失败元信息 helper
│   │   │   ├── openai_client.py    # OpenAI 客户端实现（json_object）
│   │   │   ├── pdf_draft_service.py # PDF 草稿创建 / 推断 / 过期清理 (P1+ 新增)
│   │   │   ├── pdf_extraction.py   # PDF 校验 / 文本提取 / 页面渲染 (P1+ 新增)
│   │   │   ├── effective_requirement_service.py # 评审阶段有效需求快照生成
│   │   │   ├── evidence_service.py # 产品文档证据提取 / 校验服务
│   │   │   ├── normalized_requirement_doc_service.py # 规范化需求同步预览生成
│   │   │   ├── normalized_requirement_doc_task_service.py # 规范化需求异步任务编排
│   │   │   ├── prompts/            # Prompt 模板（需求拆解 + 测试用例匹配 + 规则树会话 + 测试方案生成 + 追问分析 / PDF 提取）
│   │   │   ├── predev_analyzer.py  # 开发前风险收敛分析器
│   │   │   ├── prerelease_auditor.py # 提测前风险审计器
│   │   │   ├── knowledge_base_importer.py # knowledge_base/products 分层导入与链路索引
│   │   │   ├── product_doc_service.py # 产品文档导入/分块/更新服务 (P1+ 新增)
│   │   │   ├── requirement_context_helpers.py # 需求输入与产品知识查询文本共享 helper
│   │   │   ├── recommender.py      # 贪心推荐算法 (P1 新增)
│   │   │   ├── risk_analysis_task_service.py # 风险收敛异步任务编排
...
│   │   ├── test_clarification_review.py # 追问分析接口 / 归一化 / PDF 补充链路测试 (P1+ 新增)
...
│   │   ├── test_effective_requirement_service.py # 有效需求快照新鲜度测试
│   │   ├── test_normalized_requirement_doc.py # 规范化需求文档生成测试
│   │   ├── test_normalized_requirement_doc_async.py # 规范化需求异步任务测试
│   │   ├── test_projects_crud_api.py      # 项目 & 需求 CRUD API 测试
│   │   ├── test_recommender.py     # 推荐算法 & 推荐 API 测试 (P1 新增)
│   │   ├── test_risk_analysis_async.py # 风险收敛异步任务回归测试
...
│   │   ├── test_pdf_draft.py      # PDF 草稿接口 / 渲染 / 过期清理测试 (P1+ 新增)
...
│   │   │   ├── clarificationReview.ts # 追问分析 / PDF 草稿 API (P1+ 新增)
│   │   │   ├── client.ts           # Axios 实例 (baseURL 配置 + getErrorMessage)
│   │   │   ├── coverage.ts         # 覆盖矩阵 API
│   │   │   ├── effectiveRequirements.ts # 有效需求快照 / 需求输入 API
│   │   │   ├── normalizedRequirementDoc.ts # 规范化需求预览 API
│   │   │   ├── normalizedRequirementDocTasks.ts # 规范化需求异步任务 API
│   │   │   ├── productDocs.ts      # 产品文档 API (P1+ 新增)
│   │   │   ├── projects.ts         # 项目 & 需求 & 版本化 API
│   │   │   ├── recommendation.ts   # 回归推荐 API (P1 新增)
│   │   │   ├── riskAnalysisTasks.ts # 风险收敛异步任务 API
...
│   │   │   ├── ClarificationReview/ # 追问分析页（含 PDF 草稿与 Markdown 导出）(P1+ 新增)
│   │   │   ├── ProductDocs/        # 产品文档页 (P1+ 新增)
...
│   │   ├── utils/
│   │   │   ├── enumLabels.check.ts # 枚举标签辅助函数编译校验
...
Requirement 1 ─── N RiskAnalysisTask 0..1 ─── 1 EffectiveRequirementSnapshot
Requirement 1 ─── 0..1 NormalizedRequirementDocTask

Project 1 ─── N DiffRecord (base_requirement_id / compare_requirement_id → Requirement)
ClarificationReviewPdfDraft 1 ─── N ClarificationReviewRecord（可选通过 `source_draft_id` 关联）
```
...
| product_code | String(64) | 可选 | 关联产品标识（业务键，对应 `product_docs.product_code`） |
...
| matched_chains | Text | 可选 | 需求关联的业务链路列表（JSON 数组字符串） |
...
#### ClarificationReviewRecord (追问分析记录，P1+ 新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 记录 ID |
| input_payload_json | Text | NOT NULL | 用户输入的五段已知信息 JSON |
| rule_text | Text | NOT NULL | 当前使用的分析规则文本 |
| result_json | Text | NOT NULL | 结构化分析结果 JSON（兼容旧结构与 v2 评审结构） |
| llm_status | String(20) | NOT NULL, 默认 failed | 模型调用状态 |
| llm_provider | String(64) | 可选 | 本次命中的 provider |
| llm_message | Text | 可选 | 失败原因或补充提示 |
| source_draft_id | Integer | 可选, 索引 | 关联的 PDF 草稿 ID |
| source_meta_json | Text | 可选 | 来源元信息（文件名、是否过期、应用字段等） |
| created_at | DateTime | NOT NULL | 创建时间 |

**结果结构补充：**
- 兼容旧结构字段：`likely_historical_rules / missing_critical_rules / risk_assumptions`
- v2 评审化结果字段：`result_version=2`、`inferred_items`、`assumption_items`、扩展后的 `priority_questions_by_role`、带 `gap_type / priority / blocking_reason` 的 `known_requirement_gaps`
- 后端只维护 `新 -> 旧` 派生兼容，不做 `旧 -> 新` 反向补齐

#### ClarificationReviewPdfDraft (追问分析 PDF 草稿，P1+ 新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 草稿 ID |
| file_name | String(255) | NOT NULL | 原始 PDF 文件名 |
| file_size_bytes | Integer | NOT NULL | 文件大小 |
| page_count | Integer | NOT NULL | 页数 |
| status | String(32) | NOT NULL, 默认 pending | 草稿状态 |
| llm_status | String(20) | NOT NULL, 默认 failed | 严格提取状态 |
| llm_provider | String(64) | 可选 | 严格提取命中的 provider |
| llm_message | Text | 可选 | 严格提取告警或失败信息 |
| infer_llm_status | String(20) | 可选 | 推断补充状态 |
| infer_llm_provider | String(64) | 可选 | 推断补充命中的 provider |
| infer_llm_message | Text | 可选 | 推断补充告警或失败信息 |
| full_text_json | Text | 可选 | 全文抽取结果 |
| page_text_stats_json | Text | 可选 | 页级文本统计 |
| selected_page_indexes_json | Text | 可选 | 视觉提取选页结果 |
| rendered_page_paths_json | Text | 可选 | 渲染后的页面图片路径 |
| vision_notes_json | Text | 可选 | 多模态视觉笔记 |
| strict_result_json | Text | 可选 | 严格提取结构化字段 |
| inference_result_json | Text | 可选 | 推断补充结构化字段 |
| expires_at | DateTime | NOT NULL, 索引 | 过期时间 |
| created_at | DateTime | NOT NULL | 创建时间 |
| updated_at | DateTime | NOT NULL | 更新时间 |

**状态语义：**
- `pending` — 草稿已创建，尚未处理
- `extracting` — 正在做文本/视觉/严格提取
- `partial_success` — 文本或视觉链路部分失败，但仍生成了可用草稿
- `success` — 严格提取完成
- `failed` — 无法生成可用草稿
...
| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | String(64) | PK (UUID) | 风险项 ID |
| requirement_id | Integer | FK → requirements.id, NOT NULL | 所属需求 |
| related_node_id | String(64) | FK → rule_nodes.id, 可选 | 关联规则节点 |
| category | Enum | NOT NULL | 风险分类 |
| risk_level | Enum | NOT NULL | 风险等级 |
| description | Text | NOT NULL | 风险描述 |
| suggestion | Text | NOT NULL | 建议处理方式 |
| decision | Enum | NOT NULL, 默认 pending | 决策状态 |
| decision_reason | Text | 可选 | 决策理由 |
| decided_at | DateTime | 可选 | 决策时间 |
| risk_source | Enum | NOT NULL, 默认 rule_tree | 风险来源（规则树 / 产品知识） |
| clarification_text | Text | 可选 | 产品澄清说明 |
| doc_update_needed | Boolean | NOT NULL, 默认 false | 是否需要更新产品文档 |
| analysis_stage | Enum(review/pre_dev/pre_release) | 可选 | 最近一次命中的分析阶段 |
| validity | Enum(active/superseded/reopened/resolved) | 可选, 默认 active | 风险账本有效性状态 |
| origin_snapshot_id | Integer | FK → effective_requirement_snapshots.id, 可选 | 首次出现该风险的快照 |
| last_seen_snapshot_id | Integer | FK → effective_requirement_snapshots.id, 可选 | 最近一次命中的快照 |
| last_analysis_at | DateTime | 可选 | 最近一次被分析命中的时间 |
| created_at | DateTime | NOT NULL | 创建时间 |
...
**风险来源 (RiskSource):**
- `rule_tree` — 基于需求文本和规则树结构分析出的技术风险
- `product_knowledge` — 结合产品文档背景知识识别出的产品知识风险

**风险账本语义补充：**
- `decision` 表示人工决策结果（pending / accepted / ignored），不等于风险是否仍然有效
- `validity` 表示收敛流程中的账本状态：`active` 为当前有效，`superseded` 为后续快照未再出现，`reopened` 为关闭后再次命中，`resolved` 为提测前审计确认已消解
- `analysis_stage` 记录该风险最后一次由哪个阶段分析器写入，便于前端按评审 / 开发前 / 提测前三段筛选

#### RequirementInput (需求输入补充，风险收敛新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 输入记录 ID |
| requirement_id | Integer | FK → requirements.id, NOT NULL | 所属需求 |
| input_type | Enum | NOT NULL | 输入类型（`raw_requirement / pm_addendum / test_clarification / review_note`） |
| content | Text | NOT NULL | 输入正文 |
| source_label | String(255) | 可选 | 来源标签（如 `requirement.raw_text`） |
| created_by | String(255) | 可选 | 录入人 |
| created_at | DateTime | NOT NULL | 创建时间 |

#### EffectiveRequirementSnapshot (有效需求快照，风险收敛新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 快照 ID |
| requirement_id | Integer | FK → requirements.id, NOT NULL | 所属需求 |
| stage | Enum(review/pre_dev/pre_release) | NOT NULL | 生成阶段 |
| status | Enum(draft/confirmed) | NOT NULL | 快照状态 |
| based_on_input_ids | Text | 可选 | 参与收敛的输入 ID 列表（JSON） |
| basis_hash | String(64) | 可选 | 基于需求原文 + 正式输入计算出的哈希，用于判断快照是否过期 |
| summary | Text | 可选 | 快照摘要 |
| base_snapshot_id | Integer | FK → effective_requirement_snapshots.id, 可选 | 基于哪次快照增量生成 |
| created_at | DateTime | NOT NULL | 创建时间 |

**快照新鲜度语义：**
- `basis_hash` 会随着需求原文或正式输入变化而变化；如果当前输入重新计算出的哈希与快照不一致，则该快照视为过期
- `EffectiveSnapshotRead.is_stale` 是接口层的派生字段，不直接落库；前端用它提示“当前快照是否还能作为后续分析/文档整理依据”

#### RiskAnalysisTask (风险收敛异步任务，2026-03-19 新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 任务 ID |
| requirement_id | Integer | FK → requirements.id, NOT NULL, 唯一约束的一部分 | 所属需求 |
| stage | Enum(review/pre_dev/pre_release) | NOT NULL, 唯一约束的一部分 | 所属阶段 |
| status | Enum(queued/running/completed/failed/interrupted) | NOT NULL, 默认 queued | 当前任务状态 |
| progress_message | Text | 可选 | 当前阶段可读提示 |
| progress_percent | Integer | 可选 | 阶段式进度值（5/45/100） |
| last_error | Text | 可选 | 最近一次失败或中断原因 |
| snapshot_id | Integer | FK → effective_requirement_snapshots.id, 可选 | 本轮任务关联的快照 |
| result_json | Text | 可选 | 阶段结果 JSON（前端用于刷新恢复与展示） |
| current_task_started_at | DateTime | 可选 | 当前任务开始时间 |
| current_task_finished_at | DateTime | 可选 | 当前任务结束时间 |
| created_at | DateTime | NOT NULL | 创建时间 |
| updated_at | DateTime | NOT NULL | 更新时间 |

**状态语义 (RiskAnalysisTaskStatus):**
- `queued` — 已接受任务，等待后台线程启动
- `running` — 后台正在执行分析
- `completed` — 本轮结果已持久化
- `failed` — 本轮执行失败，保留 `last_error`
- `interrupted` — 服务重启时中断的进行中任务

**约束语义：**
- 表级唯一约束 `uq_risk_analysis_task_requirement_stage` 保证“每个需求每个阶段仅保留 1 条最新任务记录”
- 重新发起阶段分析时会复用该记录，覆盖运行态字段；若上一次成功结果仍存在，则在新任务执行中继续保留并展示

#### NormalizedRequirementDocTask (规范化需求异步任务，2026-03-19 新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 任务 ID |
| requirement_id | Integer | FK → requirements.id, NOT NULL, UNIQUE | 所属需求 |
| status | Enum(queued/running/completed/failed/interrupted) | NOT NULL, 默认 queued | 当前任务状态 |
| progress_message | Text | 可选 | 当前进度提示 |
| progress_percent | Integer | 可选 | 当前进度百分比 |
| last_error | Text | 可选 | 最近一次失败原因 |
| basis_hash | String(64) | 可选 | 发起任务时基于需求原文 + 正式输入计算出的哈希 |
| uses_fresh_snapshot | Boolean | NOT NULL, 默认 false | 是否复用了最新且未过期的有效需求快照 |
| snapshot_stale | Boolean | NOT NULL, 默认 false | 发起任务时是否检测到“有快照但已过期” |
| source_payload_json | Text | 可选 | 送给模型的原始需求输入载荷 |
| snapshot_payload_json | Text | 可选 | 送给模型的快照载荷；仅在快照新鲜时写入 |
| result_markdown | Text | 可选 | 模型生成的规范化需求 Markdown |
| llm_provider | String(64) | 可选 | 本轮使用的模型 provider |
| current_task_started_at | DateTime | 可选 | 当前任务开始时间 |
| current_task_finished_at | DateTime | 可选 | 当前任务结束时间 |
| created_at | DateTime | NOT NULL | 创建时间 |
| updated_at | DateTime | NOT NULL | 更新时间 |
...
**状态语义：**
- `queued` / `running` / `completed` / `failed` / `interrupted` 与风险收敛异步任务保持一致，便于前端统一处理状态提示
- 表级唯一约束 `uq_normalized_doc_task_requirement` 保证“每个需求仅保留 1 条最新规范化需求任务”，重复发起时会复用同一条记录

#### EffectiveRequirementField (有效需求字段，风险收敛新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 字段 ID |
| snapshot_id | Integer | FK → effective_requirement_snapshots.id, NOT NULL | 所属快照 |
| field_key | String(64) | NOT NULL | 字段名（模块、前置条件、异常处理等） |
| value | Text | 可选 | 字段值 |
| derivation | Enum | 可选 | 推导方式（`explicit / inferred / missing / contradicted`） |
| confidence | Float | 可选 | 置信度 |
| source_refs | Text | 可选 | 来源引用列表（JSON） |
| notes | Text | 可选 | 补充备注 |
| sort_order | Integer | NOT NULL, 默认 0 | 展示顺序 |

**字段展示约束：**
- `rollout_strategy` 已从评审快照的可见字段集合中移除；后端在生成 / 解析 / 返回最新快照时都会过滤该字段，前端 RiskPanel 也不会展示它

#### EvidenceBlock (产品知识证据块，风险收敛新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 证据块 ID |
| product_doc_id | Integer | FK → product_docs.id, NOT NULL | 所属产品文档 |
| chunk_id | Integer | FK → product_doc_chunks.id, 可选 | 来源分块 |
| evidence_type | Enum | NOT NULL | 证据类型（`field_rule / glossary / constraint / exception_flow`） |
| module_name | String(255) | 可选 | 归属模块名 |
| statement | Text | NOT NULL | 结构化证据陈述 |
| status | Enum(draft/verified/rejected) | NOT NULL | 证据状态 |
| source_span | Text | 可选 | 原文定位 |
| chunk_content_hash | String(64) | 可选 | 分块内容哈希 |
| created_from | Enum | 可选 | 来源（分块抽取 / 风险澄清） |
| created_at | DateTime | NOT NULL | 创建时间 |

#### ProductDoc (产品文档，P1+ 新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 文档 ID |
| product_code | String(64) | UNIQUE, NOT NULL, 索引 | 产品标识 |
| name | String(255) | NOT NULL | 产品名称 |
| description | Text | 可选 | 文档描述 |
| file_path | String(512) | 可选 | 原始文件路径 |
| version | Integer | NOT NULL, 默认 1 | 文档版本号 |
| created_at | DateTime | NOT NULL | 创建时间 |
| updated_at | DateTime | NOT NULL | 更新时间 |

#### ProductDocChunk (产品文档分块，P1+ 新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 分块 ID |
| product_doc_id | Integer | FK → product_docs.id, NOT NULL | 所属产品文档 |
| stage_key | String(64) | NOT NULL | 分块标识（如 `stage_0`） |
| title | String(255) | NOT NULL | 分块标题 |
| content | Text | NOT NULL | 分块内容 |
| sort_order | Integer | NOT NULL, 默认 0 | 排序号 |
| keywords | Text | 可选 | 关键词列表（逗号分隔） |
| chain_key | String(128) | 可选, 索引 | 分块所属链路键（`overview` / `common-concepts` / 具体业务链路） |
| source_file | String(512) | 可选 | 分块来源 Markdown 文件相对路径 |

#### ProductDocUpdate (产品文档更新建议，P1+ 新增)

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | Integer | PK, 自增 | 更新建议 ID |
| product_doc_id | Integer | FK → product_docs.id, NOT NULL | 所属产品文档 |
| chunk_id | Integer | FK → product_doc_chunks.id, 可选 | 关联分块 |
| risk_item_id | String(64) | FK → risk_items.id, 可选 | 来源风险项 |
| original_content | Text | 可选 | 原始段落内容 |
| suggested_content | Text | 可选 | 建议替换内容 |
| status | Enum | NOT NULL, 默认 pending | 审核状态 |
| reviewed_at | DateTime | 可选 | 审核时间 |
| applied_at | DateTime | 可选 | 应用时间 |
| created_at | DateTime | NOT NULL | 创建时间 |

**文档更新状态 (DocUpdateStatus):**
- `pending` — 待审核
- `approved` — 已通过并应用
- `rejected` — 已拒绝
...
**业务关联补充：**
- `Project.product_code` 通过业务键关联 `ProductDoc.product_code`，不使用数据库外键，便于项目在无产品文档时保持可空状态
- `RiskItem` 可选关联 `ProductDocUpdate`，用于记录“某条风险推动了哪次产品文档更新建议”
- `RequirementInput`、`EffectiveRequirementSnapshot`、`EvidenceBlock` 共同构成风险收敛的共享持久化层：输入先沉淀，快照再归纳，风险与证据最终通过快照 ID 和文档分块关联起来
- `RiskAnalysisTask` 是风险收敛阶段的异步编排层，不替代 `RiskItem` / `EffectiveRequirementSnapshot`，而是为它们提供可恢复的后台执行状态和最近一次结果缓存
- `NormalizedRequirementDocTask` 与风险收敛共享同一套输入 / 快照基础数据，但产物是给人阅读的 Markdown 文档，不直接回写风险账本
...
| 方法 | 路径 | 说明 | 请求体 | 响应 |
|------|------|------|--------|------|
| POST | `/api/projects` | 创建项目 | `{name, description?, product_code?}` | ProjectRead (201) |
| GET | `/api/projects` | 获取所有项目列表 (按 ID 倒序) | — | ProjectRead[] |
| GET | `/api/projects/{project_id}` | 获取单个项目详情 | — | ProjectRead |
| PUT | `/api/projects/{project_id}` | 更新项目 | `{name, description?, product_code?}` | ProjectRead |
| DELETE | `/api/projects/{project_id}` | 删除项目 (级联删除关联需求/用例/分析记录) | — | 204 No Content |
| POST | `/api/projects/{project_id}/requirements` | 创建需求 | `{title, raw_text, source_type?, matched_chains?}` | RequirementRead (201) |
...
| PUT | `/api/projects/{project_id}/requirements/{requirement_id}` | 更新需求 | `{title, raw_text, source_type?, matched_chains?}` | RequirementRead |
| DELETE | `/api/projects/{project_id}/requirements/{requirement_id}` | 删除需求 (级联删除关联规则节点/路径) | — | 204 No Content |

**关键机制：**
- 项目可选绑定 `product_code`，用于声明“该项目归属哪个产品知识库域”
- 需求可额外保存 `matched_chains`，用于显式声明“本需求涉及哪些业务链路”；评审分析与风险分析会优先按链路召回产品知识
- 风险分析时会通过需求 → 项目 → `product_code` 召回关联产品文档分块，为 LLM 提供产品背景知识
...
- 匹配优先走 LLM 批量匹配；当输入为空或规则节点为空时直接短路返回
- 正常 `llm` 模式下若所有 provider 都失败，返回空匹配结果并标记 `analysis_mode=llm_failed`
- 返回 `analysis_mode`、`llm_status`、`llm_provider`、`llm_message`、`confidence`、`match_reason`
...
- 若所有 provider 都失败、超时、返回空节点或结构不兼容，则返回空结果并标记 `llm_failed`
- 只有显式配置 `AI_PARSE_PROVIDER=mock` 时，才会使用规则分句解析
- 返回 `analysis_mode` 以标识本次解析来源：
  - `llm`：LLM 解析成功
  - `mock`：显式关闭 LLM（如 `AI_PARSE_PROVIDER=mock`）时使用规则分句
  - `llm_failed`：所有 provider 调用失败，返回空结果
...
- `llm` 模式下使用配置驱动 provider chain；默认链路可配置为 `gpt-5.4 -> gpt-5.3-codex -> zhipu`
- 分析结果会返回并持久化 `analysis_mode`：
  - `llm`：本次由 LLM 成功生成
  - `mock`：本次直接使用 Mock 引擎
  - `llm_failed`：所有 provider 调用失败，返回空结果
- 分析结果会额外返回 `llm_status / llm_provider / llm_message`
- 分析结果会返回并持久化 `llm_provider`（当启用 LLM 链路时）：
  - `main` / `backup` / `zhipu`：本次实际命中的 provider alias
  - `null`：本次未走 LLM（如 `mock`）或 provider 不可识别
- 导入接口 (`/import`) 当前仅支持导入判断树节点（`import_decision_tree`）
- 若本次分析结果为 `llm_failed`，导入接口会直接拒绝
...
| POST | `/api/rules/sessions/{session_id}/generate` | 异步启动规则树生成任务，立即返回 accepted 状态 | `multipart/form-data(requirement_text, title?, image?)` | `{accepted, session}` |
...
**RuleTreeSessionRead 关键字段：**
- `status`：`active / generating / reviewing / saving / completed / failed / interrupted / confirmed / archived`
- `progress_stage`：阶段码，前端用于轮询恢复与阶段显示
- `progress_message`：当前阶段的可读提示
- `progress_percent`：阶段映射进度值（本期仅做阶段式进度）
- `last_error`：最近一次失败或中断原因
- `generated_tree_snapshot`：生成阶段树快照
- `reviewed_tree_snapshot`：审查后树快照
- `current_task_started_at / current_task_finished_at`：当前异步任务开始/结束时间

**关键机制：**
- `generate` 已改为异步任务启动接口：请求线程只负责校验、落库、返回 `accepted: true`
- 后台任务按 `generating -> reviewing -> saving -> completed` 顺序推进，每个阶段都会立即提交数据库
- 前端通过 `GET /api/rules/sessions/{session_id}` 轮询恢复状态；刷新页面后也能从持久化状态与树快照恢复结果
- 同一会话在 `generating / reviewing / saving` 期间禁止重复启动生成，接口返回 `409`
- 后端启动时会扫描未完成任务，将其统一标记为 `interrupted`，避免页面永久停留在假“进行中”
...
| GET | `/api/requirements/{requirement_id}/analysis-tasks` | 获取三阶段最新任务汇总 | — | RiskAnalysisTaskSummaryRead |
| GET | `/api/requirements/{requirement_id}/analysis-tasks/{stage}` | 获取单阶段最新任务 | — | RiskAnalysisTaskRead \| null |
| POST | `/api/requirements/{requirement_id}/analysis-tasks/{stage}` | 异步启动阶段分析任务 | — | `{accepted, task}` |
| POST | `/api/requirements/{requirement_id}/snapshots/review` | 生成评审阶段有效需求快照（同步兼容接口） | — | ReviewSnapshotResponse |
| GET | `/api/requirements/{requirement_id}/snapshots` | 获取需求的快照历史 | — | EffectiveSnapshotRead[] |
| GET | `/api/requirements/{requirement_id}/snapshots/latest?stage=review|pre_dev|pre_release` | 获取最新快照 | — | EffectiveSnapshotRead \| null |
| POST | `/api/requirements/{requirement_id}/inputs` | 补充正式需求输入 | `{input_type, content, source_label?, created_by?}` | RequirementInputRead (201) |
| GET | `/api/requirements/{requirement_id}/inputs` | 获取正式需求输入列表 | — | RequirementInputRead[] |
| POST | `/api/ai/risks/predev-analyze` | 开发前风险收敛分析（同步兼容接口） | `{requirement_id}` | PredevAnalysisResponse |
| POST | `/api/ai/risks/prerelease-audit` | 提测前风险审计（同步兼容接口） | `{requirement_id}` | PrereleaseAuditResponse |
...
| PUT | `/api/rules/risks/{risk_id}/clarify` | 保存产品澄清说明 | `{clarification_text, doc_update_needed?}` | RiskItemRead |
...
- 风险收敛采用三阶段链路：
  - `review`：把原始需求、补充输入和产品知识证据收敛为第一版有效需求快照
  - `pre_dev`：基于最新快照和规则树做开发前冲突分析，生成冲突项与新增风险
  - `pre_release`：在提测前审计阻塞风险、重开风险和已解决风险，并更新账本状态
- 三阶段入口现已异步化：前端主流程通过 `analysis-tasks` 接口启动后台线程，再按阶段轮询最新任务状态
- `risk_analysis_tasks` 表按 `requirement_id + stage` 维度持久化最新任务，支持刷新恢复、失败回显和服务重启后的 `interrupted` 标记
- 任务结果写入 `result_json`，因此页面在重新发起分析期间仍可继续展示上一轮成功结果，不会闪空
- 若需求所属项目绑定了 `product_code`，后端会从对应产品文档中召回相关分块，补充给 LLM 作为产品背景知识
- 若需求已明确选择 `matched_chains`，`review` / `pre_dev` / `pre_release` 三阶段会直接按链路感知召回产品知识，不再额外执行模块分析
- `review` / `pre_dev` / `pre_release` 三阶段的产品知识召回查询同时包含需求原文与正式补充输入，避免补充说明仅进入 prompt、不参与召回
- 产品知识优先走 evidence-first 检索：先命中 `evidence_blocks`，不足时再回退到原始文档分块
- 规则树生成阶段当前仍只使用 `requirement.raw_text` 参与产品知识召回，这是有意识的设计决策：其召回目标是补充规则参考，不是做需求归因分析；只有当出现以下任一稳定信号时，才考虑把正式补充输入引入该链路：
  - 规则树结果持续漏掉仅存在于 `pm_addendum` / `review_note` 中的关键业务约束
  - 已显式选择 `matched_chains` 但规则树生成仍频繁召回错链路知识
  - 出现“评审快照已识别、规则树生成未识别”的稳定复现案例
- 风险项新增 `risk_source` 字段，区分 `rule_tree` 与 `product_knowledge` 两类来源；`product_knowledge` 用于标记需求与现有产品流程/约束冲突或遗漏
...
- 风险列表已从“每轮全量覆盖”切换为连续账本：分析器按风险描述归并已有记录，并基于 `validity` / `analysis_stage` / 快照引用更新状态
- 有效需求快照新增 `basis_hash`，接口返回新增 `is_stale` 派生字段；`RiskPanel` 会据此在开发前/提测前阶段提示“请先刷新评审快照”
- 旧的同步接口 `POST /snapshots/review`、`POST /api/ai/risks/predev-analyze`、`POST /api/ai/risks/prerelease-audit` 仍保留，便于测试与兼容调用；当前 `RiskPanel` 已切到异步任务接口
- 用户可对每条风险项进行决策（接受/忽略/转节点/删除）
- 已接受的风险项支持补充 `clarification_text`，并可标记 `doc_update_needed=true` 以进入产品文档维护流程
- `PUT /decision` 在 `decision=accepted` 且 `auto_create_node=true` 时，会在保存决策后立即调用 `risk_to_node`，并对所属需求执行规则路径重算；若自动转节点失败，仅记录 warning，不回滚已保存的决策结果
- "转节点"接口会将风险项直接转化为规则树节点，增强规则覆盖范围
- 删除操作通过前端 Popconfirm 二次确认后执行，物理删除风险记录

### 4.13 规范化需求文档 (`/api/requirements/{requirement_id}/normalized-doc*`，2026-03-19 新增)

| 方法 | 路径 | 说明 | 请求体 | 响应 |
|------|------|------|--------|------|
| POST | `/api/requirements/{requirement_id}/normalized-doc/preview` | 同步生成规范化需求预览 | — | NormalizedRequirementDocPreview |
| GET | `/api/requirements/{requirement_id}/normalized-doc-tasks/latest` | 获取该需求最近一次规范化需求任务 | — | NormalizedRequirementDocTaskRead \| null |
| POST | `/api/requirements/{requirement_id}/normalized-doc-tasks` | 异步启动规范化需求整理任务 | — | `{accepted, task}` |

**关键机制：**
- 规范化需求文档定位为“给产品 / 开发 / 测试共同阅读的 Markdown PRD”，不直接替代有效需求快照
- 生成时优先尝试复用最新且未过期的评审快照；若快照已过期或不存在，则退回到“需求原文 + 正式输入”实时整理
- 响应中的 `uses_fresh_snapshot` / `snapshot_stale` 会驱动前端提示“已复用最新快照 / 当前快照已过期 / 暂无快照参考”
- 前端主流程使用异步任务接口：需求详情弹窗点击“导出规范化需求”后先启动后台任务，完成后直接打开 Markdown 预览弹窗
- 当前没有单独的后端导出文件接口；下载 `.md` 文件由前端基于 `result_markdown` / `markdown` 直接生成 Blob 完成
- 后端启动时会把未完成的规范化需求任务统一恢复为 `interrupted`，避免页面长期显示伪“进行中”

### 4.14 测试方案与用例生成 (`/api/test-plan`，P1+ 新增)
...
### 4.15 产品知识库 (`/api/product-docs`，P1+ 新增)

| 方法 | 路径 | 说明 | 请求体 | 响应 |
|------|------|------|--------|------|
| POST | `/api/product-docs/import` | 导入 Markdown 产品文档 | `{product_code, name, description?, content}` | ProductDocRead (201) |
| GET | `/api/product-docs` | 获取产品文档列表 | — | ProductDocRead[] |
| GET | `/api/product-docs/{product_code}` | 获取产品文档详情（含分块） | — | ProductDocDetailRead |
| GET | `/api/product-docs/{product_code}/chains` | 获取该产品域的链路列表 | — | ChainInfo[] |
| POST | `/api/product-docs/import-knowledge-base` | 批量导入本地 `knowledge_base/products/` | — | `{imported_domains, product_codes}` |
| PUT | `/api/product-docs/{product_code}/chunks/{chunk_id}` | 更新单个文档分块 | `{content}` | ProductDocChunkRead |
| DELETE | `/api/product-docs/{product_code}` | 删除产品文档 | — | 204 No Content |
| POST | `/api/product-docs/suggest-update` | 基于风险澄清或手工补录生成文档更新建议 | `{product_doc_id, clarification_text, risk_item_id?, supplement_text?}` | ProductDocUpdateRead (201) |
| GET | `/api/product-docs/{product_code}/evidence` | 获取产品知识证据块列表 | — | EvidenceBlockRead[] |
| POST | `/api/product-docs/{product_code}/evidence/bootstrap` | 从现有文档分块初始化证据块 | — | `{created, evidences}` |
| POST | `/api/evidence/from-clarification` | 从风险澄清生成证据块 | `{risk_item_id, statement, evidence_type?, module_name?}` | EvidenceBlockRead (201) |
| PUT | `/api/evidence/{evidence_id}` | 编辑证据块 | `{statement?, evidence_type?, module_name?}` | EvidenceBlockRead |
| PUT | `/api/evidence/{evidence_id}/verify` | 确认证据块 | — | EvidenceBlockRead |
| PUT | `/api/evidence/{evidence_id}/reject` | 驳回证据块 | — | EvidenceBlockRead |
| GET | `/api/product-docs/updates/list?product_doc_id=N` | 获取文档更新建议列表 | — | ProductDocUpdateRead[] |
| PUT | `/api/product-docs/updates/{update_id}/apply` | 应用更新建议 | — | ProductDocUpdateRead |
| PUT | `/api/product-docs/updates/{update_id}/reject` | 拒绝更新建议 | — | ProductDocUpdateRead |

**关键机制：**
- 导入时按 Markdown `##` / `###` 标题自动分块，生成 `stage_key`、`sort_order` 和关键词索引
- 同一 `product_code` 再次导入时会覆盖旧分块并递增 `version`
- 本地 `knowledge_base/products/` 支持按产品域目录批量导入；每个 Markdown 文件会映射出 `chain_key / source_file / display_name`
- `GET /chains` 为需求表单提供链路多选数据源，帮助用户显式声明需求涉及的业务链路
- 风险分析通过 `product_code` 和需求文本关键词匹配召回相关分块，优先补充“术语 / 总览 / 概述 / 背景”等基础段落
- `suggest-update` 会结合风险描述、产品澄清说明和原始段落，生成一条待审核的 `ProductDocUpdate`
- `risk_item_id` 现在是可选的：除了 RiskPanel 中的风险驱动链路，也支持在 Product Docs 页面直接用 `supplement_text + clarification_text` 做手工知识补录
- 产品文档可先通过 `bootstrap` 从现有章节抽取结构化证据，再通过风险澄清持续补充证据块，供后续 review / pre_dev 分析直接检索
- 当前规则树页面中的 RiskPanel 已串联该接口：已接受风险保存澄清且勾选“需要更新产品文档”后，前端会按当前选中项目的 `product_code` 自动匹配产品文档并触发 `suggest-update`
- `apply` 会回写建议内容到原始分块、刷新关键词，并自动提升产品文档版本号
- 当前前端页面已覆盖文档导入、详情查看、分块编辑、更新记录审核，以及“手工补录知识”入口；`suggest-update` 既可由风险澄清触发，也可由产品文档维护者主动补录

### 4.16 追问分析与 PDF 草稿 (`/api/ai/clarification-review*`，P1+ 新增)

| 方法 | 路径 | 说明 | 请求体 | 响应 |
|------|------|------|--------|------|
| POST | `/api/ai/clarification-review/analyze` | 执行追问分析并持久化记录 | `{requirement_text, current_surface_flow, involved_modules, known_background, unknowns, rule_text, source_draft_id?, applied_fields?}` | `ClarificationReviewRecordRead` (201) |
| GET | `/api/ai/clarification-review/records` | 获取最近记录列表 | `?limit=20` | `ClarificationReviewRecordSummaryRead[]` |
| GET | `/api/ai/clarification-review/records/{record_id}` | 获取单条记录详情 | — | `ClarificationReviewRecordRead` |
| DELETE | `/api/ai/clarification-review/records/{record_id}` | 删除单条记录 | — | `204 No Content` |
| POST | `/api/ai/clarification-review/pdf-drafts` | 上传 PDF 并创建草稿 | `multipart/form-data: {file}` | `PdfDraftRead` (201) |
| GET | `/api/ai/clarification-review/pdf-drafts/{draft_id}` | 获取 PDF 草稿详情 | — | `PdfDraftRead` |
| POST | `/api/ai/clarification-review/pdf-drafts/{draft_id}/infer` | 基于严格提取结果生成推断补充 | — | `PdfDraftInferResponse` |

**关键机制：**
- `analyze` 要求 `rule_text` 非空，且五段输入至少有一段非空
- 当请求带 `source_draft_id` 时，后端会把 PDF 草稿中的严格提取、推断补充、视觉笔记和文档冲突整理成“PDF 补充参考材料”拼进 prompt
- 结果存储以 v2 结构为主：`result_version=2`、`inferred_items`、`assumption_items`、扩展问题项和分级缺陷；同时由后端派生旧字段以兼容旧展示
- 读取详情时会根据 `source_draft_id` 决定是否允许保留 `pdf_draft` 作为 `source_type`
- PDF 草稿默认 TTL 为 24 小时；后端启动时会自动清理过期草稿
- Markdown 导出不依赖后端独立接口，由前端基于结构化结果本地生成 `.md`
...
**目标：** 通过多轮对话式交互，由 AI 异步生成、审查和迭代优化规则树，最终确认导入正式库，并支持刷新后的进度恢复。

**核心流程：**
1. **创建会话** — 关联需求、保存需求文本快照、初始化消息历史
2. **启动生成任务** — 用户提供需求描述 → 会话立即进入 `generating`，接口返回 `accepted: true`
3. **后台执行生成任务** — 进程内后台线程完成 `LLM 生成 -> LLM 自动审查 -> 阶段落库`
3. **增量更新** — 用户提供修改意见 → LLM 基于已有树和反馈生成更新后的规则树 → 计算新旧树 diff
4. **确认导入** — 将最终树写入正式 `rule_nodes` 表，删除旧节点 → 插入新节点 → 重算规则路径

**异步状态机：**
- `active` — 会话已创建，尚未开始生成
- `generating` — 后台正在生成规则树
- `reviewing` — 后台正在审查生成结果
- `saving` — 后台正在持久化阶段结果
- `completed` — 本轮生成任务完成，可恢复查看结果
- `failed` — 本轮生成失败，`last_error` 记录原因
- `interrupted` — 后端重启时中断的进行中任务
- `confirmed` — 用户已将最终树应用到正式规则树

**前端恢复语义：**
- 抽屉会根据 `status + progress_stage + snapshots` 恢复“查看结果 / 应用到规则树”状态
- `completed` 显示生成结果与开始/结束时间
- `confirmed` 显示“已应用到规则树”和完成时间
- `failed / interrupted` 显示错误提示，并提供重新生成入口
...
### 5.12 风险识别分析与异步编排 (`risk_service.py` / `risk_analysis_task_service.py`，P1+ 新增)

**目标：** 基于需求文本和规则树结构，利用 AI 自动识别潜在风险点，并将三阶段风险收敛改造成可恢复的后台任务。

**核心流程：**
1. `risk_service.py` 负责原始风险识别：收集需求文本、规则树节点、正式补充输入、产品知识上下文，并调用 LLM 产出风险项
2. 服务层按 `requirement_id` 合并并发风险识别请求，避免同一需求短时间重复触发多次 LLM
3. 风险落库不再清空整表，而是按描述归并已有 `RiskItem`，更新 `analysis_stage / validity / origin_snapshot_id / last_seen_snapshot_id`
4. `risk_analysis_task_service.py` 负责 `review / pre_dev / pre_release` 三阶段异步任务启动、状态推进和结果持久化
5. 后台线程会按阶段路由到已有同步分析器：
   - `review` -> `generate_review_snapshot`
   - `pre_dev` -> `analyze_for_predev`
   - `pre_release` -> `audit_for_prerelease`
6. 任务执行成功后会写入 `snapshot_id + result_json`，失败则写入 `failed + last_error`，服务重启后由启动恢复逻辑统一改写为 `interrupted`
7. 用户可对风险继续做接受、忽略、转节点、删除和产品澄清；这些决策会作用于连续风险账本，而不是一次性结果集

**产品知识召回策略：**
- `requirement_context_helpers.py` 统一维护 `list_requirement_inputs` 与 `build_product_context_query_text`，让 review / risk / pre_dev / pre_release 四条路径使用一致的需求原文 + 补充输入拼装逻辑
- `risk_service.py` 在未命中 `matched_chains` 时才执行模块分析；命中链路时直接沿用链路信息召回产品知识，避免模块识别再次引入噪声
- 评审快照、原始风险识别、开发前分析、提测前审计四条路径都已接入 `get_relevant_evidence + get_chain_aware_chunks/get_relevant_chunks` 的组合召回，原始风险识别也会把“正式补充输入”显式透传给 Prompt
- `risk_analysis.py` 与 `effective_requirement.py` 的产品知识指令已扩展为四类核对目标：流程/规则/约束、历史包袱/待确认问题/跨系统交互点、多入口一致性、保存副作用/反向约束/附带写入/异步后置，并限制为“与当前需求对象、字段、保存链路、返回反馈直接相关”的条目
...
- `backend/app/main.py` 启动时会恢复 `queued/running` 风险阶段任务，将其统一标记为 `interrupted`
- `backend/tests/test_risk_service.py` 覆盖并发合并与风险账本更新逻辑；`backend/tests/test_risk_analysis_async.py` 覆盖任务表、状态接口、启动冲突和后台结果持久化

### 5.13 产品文档管理 (`product_doc_service.py`，P1+ 新增)

**目标：** 将产品背景知识以 Markdown 文档形式沉淀为可检索分块，并支持基于风险澄清生成文档更新建议。

**核心流程：**
1. 导入 Markdown 文档后按 `##` / `###` 标题分块，提取 `title / content / keywords / sort_order`
2. 同一 `product_code` 再次导入时覆盖旧分块并递增 `version`
3. `get_relevant_chunks` 采用 evidence-first + matched/related modules + 关键词评分的混合召回；优先保留“术语 / 总览 / 背景”等基础段落，并对 `主要流程 / 入口清单 / 依赖表` 做多样性限制
4. 当常规 `top-N` 未包含合格的 `related_modules` 分块时，检索层会为满足 `kw_score > 0 or evidence_score > 0` 门槛的 related 分块保留 1 个兜底位；若当前结果里没有可替换的泛化标题分块，则保持原结果不变并记录诊断日志
5. `_calc_kw_score(title, keywords, req_keywords)` 统一复用于常规排序、related 兜底门槛和 `get_chain_aware_chunks` 的 Tier 3 打分，确保标题重叠与关键词重叠使用同一评分口径
6. `suggest_doc_update` 基于风险描述、人工澄清说明和原始段落生成一条 `ProductDocUpdate`
7. `apply_doc_update` 将建议内容写回原始分块，刷新关键词并同步提升文档版本号
8. `reject_doc_update` 仅更新建议状态，不修改原始文档内容

### 5.14 测试方案与用例生成 (`test_plan_generator.py`，P1+ 新增)
...
### 5.15 追问分析与 PDF 草稿归一化 (`clarification_review_service.py` / `pdf_draft_service.py`，P1+ 新增)

**追问分析主链路：**
1. 用户输入五段已知信息 + `rule_text`，可选挂一份 `source_draft_id`
2. 若存在 PDF 草稿，后端将严格提取字段、推断补充字段、视觉笔记、文档冲突压缩进 `PDF_SUPPLEMENT_SECTION`
3. LLM 只输出顶层扁平的新结构：`result_version=2`、`inferred_items`、`assumption_items`、扩展问题项、扩展缺陷项、`summary_markdown`
4. `_normalize_result()` 负责：
   - 结构合法性校验
   - `source_type / answer_format / gap_type / priority` 归一化
   - `P0` 必填 `blocking_reason` 校验
   - 按固定阈值做 `P0` 软降级
   - 从新结构派生旧字段，兼容旧记录和旧导出

**PDF 草稿链路：**
1. `validate_pdf_file()` 校验扩展名、PDF 头、页数上限
2. `extract_full_text()` 读取全文；`compute_page_stats()` 和 `select_vision_pages()` 选出低文本页供多模态补充
3. `render_pages()` 将选定页渲染为图片，供视觉模型生成 `vision_notes`
4. 严格提取阶段输出结构化五段字段和冲突；推断补充阶段在严格提取基础上补全可能缺失的信息
5. `cleanup_expired_drafts()` 在应用启动时执行，清理超过 TTL 的临时草稿和磁盘文件

**结果评审化补充：**
- `known_requirement_gaps` 统一承载“逻辑缺口 / 流程缺失 / 边界未定义 / 数据缺失 / 规则缺失”
- `missing_critical_rules` 不再是新结构主字段，而是从 `gap_type == rule_missing` 派生出来的兼容字段
- 前端新版展示与导出统一以 `result_version === 2` 作为切换条件
...
- 已集中维护风险等级、节点类型、用例状态、来源类型、需求输入类型、导入分析模式、需求拆解分析模式、回归模式、风险任务状态、测试点类型等中文标签映射
- 各页面统一通过 `get*Label()` 辅助函数渲染用户可见文本；入参为空返回 `"-"`，未知值回退原始枚举值
- `enumLabels.check.ts` 作为轻量编译校验文件，确保新增辅助函数导出后不会被遗漏引用

### 6.2 页面功能矩阵

| 页面 | 路由 | 核心功能 |
|------|------|----------|
| **项目与需求** | `/` | 项目 CRUD (列表/新建/查看/编辑/删除)、需求 CRUD (列表/新建/查看/编辑/删除)、规范化需求导出 |
...
| **产品知识库** | `/product-docs` | 浏览本地知识库产品域、按链路查看 Markdown 内容、为需求选择业务链路提供数据源 |
| **需求拆解** | `/architecture` | 上传流程图和/或填写描述进行 AI 分析，展示判断树并支持导入到规则树 |
| **追问分析** | `/clarification-review` | 自由输入老项目背景做追问分析，支持 PDF 草稿导入、结构化评审结论展示与 Markdown 导出 |
...
**项目操作：**
1. 点击「新建项目」→ Modal 表单 (名称 + 描述 + 所属产品)
2. 项目列表每行含「查看」「修改」「删除」操作按钮
3. 「查看」→ Descriptions 详情弹窗 (只读)
4. 「修改」→ Modal 表单 (预填当前值，支持改绑所属产品)
5. 「删除」→ Popconfirm 确认后调 DELETE API (级联删除)
6. 左侧项目卡片若已绑定产品文档，会展示 `product_code` 标签
7. 点击项目行 → 设为当前选中项目，右栏自动加载该项目需求

**需求操作：**
1. 点击「新建需求」→ Modal 表单 (标题 + 原文 + 关联业务链路，可选)
2. 需求表格展示 ID / 标题 / 版本 (Tag `vN`) / 来源类型 (中文标签)，每行含「查看」「修改」「删除」
3. 若当前项目已绑定产品知识库，前端会调用 `GET /api/product-docs/{product_code}/chains` 加载可选业务链路
4. 「修改」→ Modal 表单含来源类型 Select (需求文档/流程图/接口文档) 和 `matched_chains` 多选
5. 点击需求行 → 设为当前选中需求
6. 点击「查看」→ 打开需求详情弹窗，底部提供「导出规范化需求」按钮
7. 点击「导出规范化需求」后，前端先调用 `POST /api/requirements/{requirement_id}/normalized-doc-tasks` 发起后台任务
8. 若任务处于 `queued/running`，详情弹窗展示“后台生成中”提示；重新打开详情时会通过 `GET /normalized-doc-tasks/latest` 自动恢复状态
9. 若任务已完成，详情弹窗展示“上次已生成完成”提示；再次点击按钮会直接打开预览
10. 预览弹窗使用 `ReactMarkdown + remark-gfm` 渲染模型返回的 Markdown，并根据 `uses_fresh_snapshot / snapshot_stale` 展示快照新鲜度提示
11. 点击「下载 Markdown」后，前端基于预览内容生成本地 `.md` 文件，不依赖额外导出接口
...
**风险识别面板 (P1+ 新增)：**
19. 工具栏新增「风险识别」按钮，点击后从页面右侧打开覆盖式 RiskPanel，避免继续压缩中间规则树画布
20. 面板顶部同时提供一个原始「分析」按钮和三个阶段按钮：`评审分析`、`开发前分析`、`提测前审计`
21. 原始「分析」仍走同步风险识别接口，用于产出基础风险台账；若需求所属项目已绑定产品文档，分析时会一并利用产品背景知识识别 `product_knowledge` 风险
22. 三阶段按钮改为异步后台任务：点击后立即返回 accepted 状态，前端按 `analysis-tasks` 接口轮询，并在页面刷新后恢复任务状态
23. 每个阶段会展示独立状态条，区分 `排队中 / 执行中 / 已完成 / 失败 / 已中断`；若任务重跑中已有历史结果，面板会继续展示上一次成功结果并提示“后台重新分析中”
24. 风险列表按来源分组展示：`rule_tree`（技术风险）与 `product_knowledge`（产品知识风险）
25. 面板头部提供全局 `接受时自动创建节点` 开关，默认开启；点击「接受」时会将 `auto_create_node=true` 一并提交到风险决策接口
26. 当接受风险且自动建节点开关开启时，后端会立即把该风险转成异常节点并重算规则路径；前端在决策成功后通过 `onRiskConverted` 触发规则树刷新
27. 每条风险可操作：接受 / 忽略 / 转为规则节点 / 删除；已接受风险还可补充“产品澄清”并标记“需更新文档”
28. 在“产品澄清”弹窗中勾选“需要更新产品文档”后，前端会先调用 `PUT /api/rules/risks/{risk_id}/clarify` 保存澄清，再根据当前选中项目的 `product_code` 查找匹配的产品文档并调用 `POST /api/product-docs/suggest-update`
29. 若当前项目未绑定产品文档，或未找到匹配 `product_code` 的文档，只提示 warning，不影响本次澄清保存
30. 删除操作通过 Popconfirm 二次确认后执行，物理删除该风险记录
31. 风险高亮：存在风险的节点在思维导图中以特殊标记显示（通过 `dataAdapter` 中的 `nodeRiskMap` 实现）
32. 存在待处理（pending）风险项时，「分析」按钮禁用并提示"请先处理所有待处理风险项"
33. 风险分析完成、阶段任务完成或决策处理后，通过 `onRisksChange` 回调同步更新父组件的风险状态，并按需刷新最新快照与正式输入列表
34. 风险面板左侧提供拖拽把手，支持在约 `340px` 到视口 `75%` 之间手动调宽；面板头部保留关闭按钮，方便在“查看风险细节”和“回看规则树”之间快速切换

**测试方案抽屉 (P1+ 新增)：**
27. 点击「AI 生成测试方案」→ 打开右侧抽屉，展示三步流程：生成测试方案 → 审核测试方案 → 生成并导入用例
28. 第一步支持两种入口：直接调用 AI 生成测试方案，或导入本地 `.md` 文件作为测试方案初稿；导入后会自动创建新会话并进入编辑态
29. 规则树工具栏支持导出 Markdown，可将当前规则树导出为 Markdown 文本，作为手工编写测试方案的骨架
30. 审核阶段支持直接编辑 Markdown 和测试点列表；当当前没有测试点时，页面展示信息提示，允许用户手动补充测试点，或直接继续生成用例
31. 直接生成用例时，AI 会根据测试方案 Markdown 自动提取测试点，不强依赖前一步已存在结构化测试点
32. 抽屉保留历史会话列表，支持查看历史方案、恢复已生成用例并重新导入
...
### 6.9 产品知识库页面交互流程 (P1+ 新增)

**页面布局：**
- 左栏：产品知识库域列表
- 右栏：当前知识库域详情、按链路分组的 Markdown 内容预览

**操作流程：**
1. 应用启动时自动扫描本地 `knowledge_base/products/` 目录，并把各产品域 Markdown 同步到 `product_docs / product_doc_chunks`
2. 左侧列表展示产品域名称、`product_code`、版本号 `vN`
3. 点击列表项 → 加载知识库详情；右侧展示产品标识、版本、描述、分块数
4. 详情区按 `chain_key` 构造成树：优先展示 `overview`（系统事实档案），其次展示具体业务链路，再展示 `common-concepts`
5. 选择某条链路后，右侧使用 `ReactMarkdown + remark-gfm` 渲染该链路下的 Markdown 片段，支持表格等 GFM 内容
6. 页面当前以浏览和核对为主，不再承担手工导入、在线逐段编辑和审核更新建议入口；文档更新建议仍主要来自 RiskPanel 的“产品澄清 -> 需要更新产品文档”链路

### 6.10 追问分析页面交互流程 (P1+ 新增)

**页面布局：**
- 左栏：最近记录列表，支持查看、切换、删除
- 中栏：自由输入表单 + PDF 草稿卡片
- 右栏：结构化分析结果、问题清单、缺陷分级、摘要与导出按钮

**操作流程：**
1. 用户填写 `需求原文 / 当前表面流程 / 涉及模块 / 已知背景 / 我暂时不知道的内容 / 分析规则`
2. 可选点击“导入 PDF”上传文档，生成 PDF 草稿
3. 草稿完成后，页面展示：
   - 严格提取结果（五段字段 + 证据）
   - 可选的推断补充结果
   - 文档冲突提示
4. 用户可把草稿字段按“保留当前 / 使用 PDF / 追加合并”回填到表单
5. 点击“开始分析”后，后端持久化一条追问分析记录；若分析成功，右栏展示结构化结果
6. 结果页按 `result_version` 分流：
   - 旧记录：展示“推测的历史规则 / 关键缺失规则 / 风险假设”
   - v2 记录：展示“合理推断 / 风险假设 / 必须补齐的规则答案 / P0-P2 缺陷”
7. 问题清单按角色分组展示，支持 `AI 补充` 角色；问题项会额外展示 `必须产出` 和 `答案形式`
8. 摘要区支持一键复制；当 `llm_status=success` 时，支持导出 Markdown
9. 导出逻辑在前端本地完成：
   - 旧记录导出旧版结构
   - `result_version=2` 导出评审结论版 Markdown
   - `gap_type`、`source_type`、`answer_format` 都会被映射成中文标签
...
- `ensure_hierarchical_knowledge_columns(engine)` — 补充 `requirements.matched_chains` 以及 `product_doc_chunks.chain_key/source_file` 字段与索引
- `ensure_clarification_review_source_columns(engine)` — 补充 `clarification_review_records.source_draft_id/source_meta_json` 字段及索引

另外，后端启动阶段会自动执行本地知识库同步：
- 扫描 `knowledge_base/products/`
- 按产品域导入 `ProductDoc`
- 按 Markdown 标题切分 `ProductDocChunk`
- 为每个分块补齐 `chain_key / source_file / stage_key`，供链路感知召回和前端链路树展示使用

确保已有数据库兼容新版本。同时自动创建 `uploads/` 目录用于文件存储，并通过 `StaticFiles` 挂载 `/uploads` 路径提供静态文件访问。

**追问分析补充：**
- 后端启动时会执行 `cleanup_expired_drafts(db)`，清理超过 24 小时的 PDF 草稿和对应临时文件
- PDF 草稿临时文件默认落在仓库根目录下的 `.tmp/pdf_drafts/`
...
> 若要使用追问分析的 PDF 草稿能力，需确保 `pypdf` 与 `pypdfium2` 已按 `requirements.txt` 安装；若缺失，追问分析主链路仍可运行，但 PDF 渲染/文本提取相关能力会在运行时抛出依赖缺失错误。
...
- `test_clarification_review.py` — 追问分析接口、v2 结果归一化、PDF 补充拼接、`pdf_draft` 来源保留测试
...
- `test_pdf_draft.py` — PDF 草稿接口、文件校验、页面渲染、过期处理测试
...
| LLM_PROVIDER_CHAIN | (空) | 配置驱动 provider 链，例如 `main,backup,zhipu` |
| LLM_PROVIDER_<ALIAS>_TYPE | (空) | provider 类型，当前支持 `openai_compatible` / `zhipu` |
| LLM_PROVIDER_<ALIAS>_API_KEY | (空) | 对应 provider 的 API Key |
| LLM_PROVIDER_<ALIAS>_BASE_URL | https://api.openai.com/v1 | `openai_compatible` provider 的基础地址 |
| LLM_PROVIDER_<ALIAS>_API_URL | provider 默认值 | `zhipu` provider 的 completions 地址 |
| LLM_PROVIDER_<ALIAS>_TEXT_MODEL | provider 默认值 | 对应 provider 的文本模型 |
| LLM_PROVIDER_<ALIAS>_VISION_MODEL | 跟随 TEXT_MODEL | 对应 provider 的多模态模型 |
| LLM_PROVIDER_<ALIAS>_THINKING_TYPE | disabled | 智谱 provider 的 `thinking` 参数开关 |
...
- `main` 使用 `openai_compatible + gpt-5.4`
- `backup` 使用 `openai_compatible + gpt-5.3-codex`
- `zhipu` 使用 `glm-4.7`
- `OPENAI_* / ZHIPU_*` 仍保留兼容能力，主要用于旧脚本和迁移期回退；新部署应优先使用 `LLM_PROVIDER_CHAIN`
...
- 建议将 `.env`（本地）设置为：
  ...
  - `LLM_PROVIDER_BACKUP_TYPE=openai_compatible`
  - `LLM_PROVIDER_BACKUP_TEXT_MODEL=gpt-5.3-codex`
  - `LLM_PROVIDER_ZHIPU_TYPE=zhipu`
  - `LLM_PROVIDER_ZHIPU_TEXT_MODEL=glm-4.7`
...
API 层 (api/)
  ...
  ├── 风险识别同步接口单独覆盖超时为 180s；三阶段风险收敛已迁移为短请求启动 + 轮询恢复
  ...
  ├── coverage — 覆盖矩阵查询
  ├── effectiveRequirements — 有效需求快照 / 需求输入
  ├── riskAnalysisTasks — 风险收敛异步任务启动 / 查询 / 轮询
  ├── risks — 原始风险识别 + 风险决策 / 澄清 / 转节点
  ├── productDocs — 产品文档 / 更新建议 / 证据块
  ├── ruleTreeSession — 规则树会话式 AI 生成
...
| **P1+ (已实现)** | ~~产品文档与产品知识~~ — Markdown 产品文档导入/分块/编辑、项目绑定产品、风险分析召回产品知识 |
...
#### 2026-04-14 — 知识库同步追问分析与 PDF 草稿能力

**范围：**
- 文档

**改动摘要：**
- 补充“追问分析 / PDF 草稿”功能在项目概述、目录结构、技术栈、页面矩阵中的位置
- 新增 `ClarificationReviewRecord`、`ClarificationReviewPdfDraft` 数据模型说明，以及 `/api/ai/clarification-review*` 接口文档
- 同步 v2 评审化结果结构、Markdown 导出、`result_version` 切换逻辑和本地 PDF 依赖说明

**涉及文件：**
- `KNOWLEDGE_BASE.md`

**接口/模型变更：**
- API: `POST /api/ai/clarification-review/analyze`
- API: `GET /api/ai/clarification-review/records`
- API: `POST /api/ai/clarification-review/pdf-drafts`
- API: `POST /api/ai/clarification-review/pdf-drafts/{draft_id}/infer`
- Model: `ClarificationReviewRecord`（新增文档说明）
- Model: `ClarificationReviewPdfDraft`（新增文档说明）
...
**兼容性说明：**
- 本次仅同步知识库文档，不修改运行时代码
- 文档已明确：追问分析结果采用 v2 新结构为主，后端保留 `新 -> 旧` 派生兼容

#### 2026-03-19 — 知识库同步规范化需求导出与快照新鲜度

**范围：**
- 文档

**改动摘要：**
- **规范化需求能力入库**：补充 `NormalizedRequirementDocTask` 数据模型、`normalized-doc` / `normalized-doc-tasks` 接口和后台恢复机制说明
- **快照新鲜度语义补齐**：记录 `EffectiveRequirementSnapshot.basis_hash` 与 `EffectiveSnapshotRead.is_stale` 的用途，说明风险面板和规范化需求整理如何消费该状态
- **项目页交互同步**：更新需求详情里的“导出规范化需求”入口、任务恢复、Markdown 预览与本地下载流程

**涉及文件：**
- `KNOWLEDGE_BASE.md`

**接口/模型变更：**
- API: `POST /api/requirements/{requirement_id}/normalized-doc/preview`
- API: `GET /api/requirements/{requirement_id}/normalized-doc-tasks/latest`
- API: `POST /api/requirements/{requirement_id}/normalized-doc-tasks`
- Model: `NormalizedRequirementDocTask`（新增文档说明）
- Model: `EffectiveRequirementSnapshot.basis_hash` / `EffectiveSnapshotRead.is_stale`（补充文档说明）
...
**兼容性说明：**
- 本次为知识库同步，不改动运行时代码语义
- 规范化需求下载目前由前端基于预览结果生成文件，后端没有新增独立导出文件接口

#### 2026-03-19 — 知识库同步风险收敛异步任务与枚举本地化

**范围：**
- 文档

**改动摘要：**
- **风险收敛异步任务入库**：补充 `RiskAnalysisTask` 数据模型、`risk_analysis_tasks` 目录结构、`analysis-tasks` API 与启动恢复机制说明
- **规则树风险面板同步**：将 RiskPanel 的“原始分析 + 三阶段后台任务 + 刷新恢复 + 保留上次成功结果”交互更新到页面说明
- **前端显示层同步**：补充 `riskAnalysisTasks.ts`、`enumLabels.check.ts` 和枚举标签集中化的现状，修正旧的输入类型与字段派生描述

**涉及文件：**
- `KNOWLEDGE_BASE.md`

**接口/模型变更：**
- API: `GET /api/requirements/{requirement_id}/analysis-tasks`
- API: `GET /api/requirements/{requirement_id}/analysis-tasks/{stage}`
- API: `POST /api/requirements/{requirement_id}/analysis-tasks/{stage}`
- Model: `RiskAnalysisTask`（新增文档说明）

**验证记录：**
- `命令`: `git diff --check`
- `结果`: 通过
- `命令`: `cd backend && ./.venv/bin/python -m pytest tests/test_api_smoke.py tests/test_risk_analysis_async.py tests/test_risk_service.py -v`
- `结果`: 通过（`56 passed`）
- `命令`: `cd frontend && npm run build`
- `结果`: 通过（保留既有第三方 `'use client'` 指令忽略提示与 chunk size warning，不影响本次改动）

**兼容性说明：**
- 旧的同步三阶段接口仍保留为兼容入口，但前端主流程已迁移到 `analysis-tasks`
- 本次为知识库同步，不引入新的运行时数据迁移

#### 2026-03-17 — 风险接受自动转节点与文档更新建议联动

#### 2026-03-18 — 风险收敛剩余链路补齐

**范围：**
- 后端 / 前端 / 文档 / 测试

**改动摘要：**
- **共享持久化模型补齐**：新增 `RequirementInput`、`EffectiveRequirementSnapshot`、`EffectiveRequirementField`、`EvidenceBlock` 等表与接口，支持正式输入、有效需求快照、产品知识证据的持续沉淀
- **三阶段分析闭环落地**：规则树 RiskPanel 新增 `评审分析`、`开发前分析`、`提测前审计` 三个入口，前端可查看快照摘要、风险有效性标签、开发前冲突、提测前审计摘要
- **连续风险账本**：`RiskItem` 新增 `analysis_stage`、`validity`、`origin_snapshot_id`、`last_seen_snapshot_id`、`last_analysis_at`，风险不再按分析轮次整体清空，而是跨阶段持续跟踪
- **产品知识手工补录**：Product Docs 页面新增“手工补录知识”入口，允许在没有 `risk_item_id` 的情况下直接补录产品知识并生成待审核文档更新建议
- **验证与文档同步**：补充风险收敛 smoke tests，并同步更新知识库和项目描述，确保剩余执行方案与当前代码一致

**涉及文件：**
- `backend/app/api/effective_requirements.py`
- `backend/app/api/evidence_blocks.py`
- `backend/app/api/requirement_inputs.py`
- `backend/app/api/product_docs.py`
- `backend/app/api/projects.py`
- `backend/app/models/entities.py`
- `backend/app/schemas/risk.py`
- `backend/app/schemas/risk_convergence.py`
- `backend/app/services/effective_requirement_service.py`
- `backend/app/services/evidence_service.py`
- `backend/app/services/predev_analyzer.py`
- `backend/app/services/prerelease_auditor.py`
- `frontend/src/api/effectiveRequirements.ts`
- `frontend/src/pages/ProductDocs/index.tsx`
- `frontend/src/pages/RuleTree/RiskPanel.tsx`
- `frontend/src/types/index.ts`

**接口/模型变更：**
- API: `POST /api/requirements/{requirement_id}/snapshots/review` (新增)
- API: `POST /api/ai/risks/predev-analyze` (新增)
- API: `POST /api/ai/risks/prerelease-audit` (新增)
- API: `POST /api/product-docs/suggest-update` 支持 `risk_item_id?` 和 `supplement_text?`
- Model: `RiskItem` 新增 `analysis_stage` / `validity` / `origin_snapshot_id` / `last_seen_snapshot_id` / `last_analysis_at`

**验证记录：**
- `命令`: `cd backend && .venv/bin/python -m pytest tests/test_api_smoke.py tests/test_risk_service.py tests/test_projects_crud_api.py -v`
- `结果`: 通过（`53 passed`）
- `命令`: `cd frontend && npm run build`
- `结果`: 通过（保留既有第三方 `'use client'` 指令忽略提示与 chunk size warning，不影响本次改动）

**兼容性说明：**
- 旧的 `POST /api/ai/risks/analyze` 和 RiskPanel「分析」按钮仍保留，作为原始风险识别入口
- 手工知识补录是新增入口，不影响既有“风险澄清 -> 文档建议”路径
- 风险列表新增阶段与有效性字段后，旧数据会在启动时按默认语义补齐，不要求人工迁移现有 SQLite 数据

**范围：**
- 后端 / 前端 / 文档

**改动摘要：**
- **风险接受默认自动建节点**：规则树 RiskPanel 头部新增全局 `接受时自动创建节点` 开关，默认开启；接受风险时不再在弹窗内单独勾选，而是直接沿用面板当前开关状态
- **自动建节点补齐路径重算**：`PUT /api/rules/risks/{risk_id}/decision` 在 `accepted + auto_create_node=true` 场景下，后端会在风险转节点后立即重算所属需求的规则路径，确保新建异常节点立刻参与路径覆盖与后续分析
- **产品澄清直连文档建议**：已接受风险在“产品澄清”中勾选“需要更新产品文档”后，前端会自动查找当前项目绑定的产品文档并调用 `POST /api/product-docs/suggest-update` 生成待审核更新建议
- **知识库同步**：补充风险接口机制、规则树页面交互和产品文档页面交互说明，避免文档与当前实现脱节
...
**接口/模型变更：**
- API: `PUT /api/rules/risks/{risk_id}/decision` 的 `auto_create_node` 现已串联“自动转节点 + 规则路径重算”完整链路
- API: `POST /api/product-docs/suggest-update` 现已接入规则树 RiskPanel 的产品澄清操作流
- Model: 无新增模型，仅补齐既有字段/接口的实际行为说明
...
**兼容性说明：**
- 未关闭开关时，接受风险的交互从“弹窗内按条勾选”改为“面板级默认策略”，接口形态不变
- 自动转节点失败不会阻断风险决策结果保存，但会记录 warning，便于后续排查
- 若项目未绑定产品文档或找不到匹配文档，风险澄清仍可正常保存，仅不会自动生成文档更新建议

#### 2026-03-23 — 链路感知产品知识库与评审召回修正

#### 2026-03-24 — 三阶段产品知识召回对齐与待确认事项格式修正

#### 2026-03-25 — 产品知识召回强化与检索契约收口

**范围：**
- 后端 / 测试 / 文档 / 知识库

**改动摘要：**
- **共享查询文本 helper 抽离**：新增 `backend/app/services/requirement_context_helpers.py`，统一承载 `list_requirement_inputs` 与 `build_product_context_query_text`，避免 `risk_service` 反向依赖 `effective_requirement_service` 形成循环引用
- **review / risk / pre_dev / pre_release 召回对齐**：评审快照补上 `get_relevant_evidence`，原始风险识别补齐 richer query、结构化证据和“正式补充输入” Prompt 区块，开发前分析与提测前审计的 `max_chunks` 从 4 调整为 6
- **Prompt 核对范围扩展**：`risk_analysis.py` 与 `effective_requirement.py` 的产品知识核对要求从“流程 / 规则 / 约束”扩展为四类：流程规则约束、历史包袱/待确认问题/跨系统交互点、多入口一致性、保存副作用/反向约束/附带写入/异步后置，并增加“仅针对与当前需求对象、字段、保存链路、返回反馈直接相关的条目输出风险”的边界
- **产品知识检索契约收口**：`get_relevant_chunks` 新增泛化标题多样性限制（`主要流程 / 入口清单 / 依赖表` 每类最多保留 2 条），并为满足 `kw_score > 0 or evidence_score > 0` 的 `related_modules` 分块保留 1 个兜底位；若结果集中没有可替换的泛化标题分块，则保持原结果不变并记录诊断日志
- **关键词打分统一**：新增 `_calc_kw_score(title, keywords, req_keywords)` 纯函数，统一复用于常规排序、related 兜底门槛和链路召回 Tier 3 打分，避免标题重叠与关键词重叠口径不一致
- **回归测试补齐**：新增 review/risk 的 evidence 与 Prompt 断言、related_modules 兜底、泛化标题多样性、title-only overlap 和“无泛化标题不强行替换”边界测试，锁住本轮检索与 Prompt 契约

**涉及文件：**
- `backend/app/services/effective_requirement_service.py`
- `backend/app/services/normalized_requirement_doc_service.py`
- `backend/app/services/normalized_requirement_doc_task_service.py`
- `backend/app/services/predev_analyzer.py`
- `backend/app/services/prerelease_auditor.py`
- `backend/app/services/product_doc_service.py`
- `backend/app/services/prompts/effective_requirement.py`
- `backend/app/services/prompts/risk_analysis.py`
- `backend/app/services/requirement_context_helpers.py`
...
**验证记录：**
- `命令`: `backend/.venv/bin/pytest backend/tests/test_effective_requirement_service.py backend/tests/test_risk_service.py backend/tests/test_evidence_service.py backend/tests/test_normalized_requirement_doc.py backend/tests/test_normalized_requirement_doc_async.py -q`
- `结果`: 通过（`88 passed`）
- `命令`: `backend/.venv/bin/pytest backend/tests/test_evidence_service.py -q`
- `结果`: 通过（`24 passed`）
- `命令`: `backend/.venv/bin/pytest backend/tests/test_risk_service.py -q`
- `结果`: 通过（`38 passed`）

**兼容性说明：**
- 未配置 `matched_chains` 的需求仍沿用模块分析 + evidence + 关键词兜底的召回路径，只是在检索层增加了 `related_modules` 兜底和泛化标题多样性约束
- 已配置 `matched_chains` 的需求继续优先走链路感知召回，不受本次 `get_relevant_chunks` 契约收口影响
- 本次不涉及 API 契约、数据库迁移或前端页面结构调整；实际生成效果仍需在真实需求上继续联调验证

**范围：**
- 后端 / 测试 / 文档 / 知识库

**改动摘要：**
- **统一产品知识查询文本**：将评审阶段构造的产品知识召回查询文本提炼为 `build_product_context_query_text`，`pre_dev` 与 `pre_release` 阶段复用同一拼装逻辑，确保需求原文与补充输入在三阶段召回中保持一致
- **链路命中时跳过模块分析**：开发前分析与提测前审计在 `Requirement.matched_chains` 已命中时不再额外执行模块识别，直接按链路召回产品证据与文档分块，减少无关模块噪声
- **结构化证据增加链路过滤**：`get_relevant_evidence` 新增 `matched_chains` 过滤能力，只保留命中链路以及 `overview` / `common-concepts` 通用分块下的证据，避免证据池跨链路串扰
- **规范化需求文档格式收口**：更新规范化需求 Prompt，明确“## 5. 待确认事项”下的每条事项都必须用 `###` 小标题承载摘要，并在标题后紧跟影响范围，便于产物统一解析和阅读
- **仓库忽略规则修正**：移除 `.gitignore` 中对整个 `backend/tests/` 目录的忽略，避免后续新增测试文件被静默排除出版本控制
...
- `backend/app/services/predev_analyzer.py`
- `backend/app/services/prerelease_auditor.py`
- `backend/app/services/prompts/normalized_requirement_doc.py`
- `backend/tests/test_risk_service.py`
- `.gitignore`
- `KNOWLEDGE_BASE.md`
...
- `命令`: `git diff --check`
- `结果`: 通过
- `命令`: `pytest backend/tests/test_effective_requirement_service.py backend/tests/test_risk_service.py -q`
- `结果`: 通过（`44 passed, 1 warning`）

**兼容性说明：**
- 未配置 `matched_chains` 的需求仍沿用原有模块分析兜底路径，不改变历史项目的召回入口
- 已配置 `matched_chains` 的需求会收窄到对应链路及通用知识块，召回结果更聚焦，但不会影响无链路标记的数据读取
- 本次不涉及 API 契约、数据库迁移或前端页面结构调整
...
**改动摘要：**
- **本地知识库分层导入**：新增 `knowledge_base/products/` 扫描与批量导入逻辑，启动时自动把产品域 Markdown 同步到 `ProductDoc / ProductDocChunk`，并为分块补齐 `chain_key`、`source_file`
- **需求链路显式绑定**：`Requirement` 新增 `matched_chains`，项目与需求页面支持按产品域加载链路列表，并在新建/编辑需求时多选业务链路
- **链路感知召回落地**：评审阶段产品知识召回改为优先使用 `matched_chains`，命中时跳过模块分析；召回查询同时拼接需求原文与正式补充输入
- **三阶段召回对齐**：`pre_dev` / `pre_release` 产品知识召回同步对齐到链路感知逻辑；规则树生成阶段保留“仅使用需求原文召回”的显式决策，并记录后续启动改造的触发条件
- **评审快照字段收口**：`rollout_strategy` 不再进入可见有效需求字段，后端生成/解析和前端 RiskPanel 展示均已同步过滤
- **产品知识库页面重构**：`/product-docs` 从手工导入/编辑页调整为知识库浏览页，支持按链路树查看 Markdown 内容，并用 `remark-gfm` 正确渲染表格
- **仓库整理**：补充 `.gitignore`，忽略本地 `knowledge_base/` 与 `docs/` 目录中的公司敏感资料及临时环境残留
...
- `backend/app/main.py`
- `backend/app/models/entities.py`
- `backend/app/schemas/product_doc.py`
- `backend/app/schemas/project.py`
- `backend/app/services/effective_requirement_service.py`
- `backend/app/services/knowledge_base_importer.py`
- `backend/app/services/product_doc_service.py`
- `backend/tests/test_effective_requirement_service.py`
- `backend/tests/test_risk_service.py`
- `frontend/package.json`
- `frontend/package-lock.json`
- `frontend/src/App.tsx`
- `frontend/src/api/productDocs.ts`
- `frontend/src/api/projects.ts`
- `frontend/src/pages/ProductDocs/index.tsx`
- `frontend/src/pages/ProjectList/index.tsx`
- `frontend/src/pages/RuleTree/RiskPanel.tsx`
- `frontend/src/types/index.ts`
- `frontend/scripts/check-product-doc-gfm.mjs`
- `frontend/scripts/check-remark-gfm-compat.mjs`
- `.gitignore`
- `KNOWLEDGE_BASE.md`

**接口/模型变更：**
- API: `GET /api/product-docs/{product_code}/chains` (新增)
- API: `POST /api/product-docs/import-knowledge-base` (新增)
- API: `POST /api/projects/{project_id}/requirements` / `PUT /api/projects/{project_id}/requirements/{requirement_id}` 请求体新增 `matched_chains?`
- Model: `Requirement` 新增 `matched_chains`
- Model: `ProductDocChunk` 新增 `chain_key` / `source_file`

**验证记录：**
- `命令`: `git diff --check`
- `结果`: 通过
- `命令`: `pytest backend/tests/test_effective_requirement_service.py backend/tests/test_risk_service.py -q`
- `结果`: 通过（`42 passed, 1 warning`）
- `命令`: `node frontend/scripts/check-product-doc-gfm.mjs`
- `结果`: 通过（`PASS: GFM table markdown rendered successfully`）
- `命令`: `node frontend/scripts/check-remark-gfm-compat.mjs`
- `结果`: 通过（`PASS: remark-gfm rendered table markup`）
- `命令`: `cd frontend && npm run build`
- `结果`: 通过（保留既有第三方 `use client` bundling 提示与 chunk size warning，不影响本次改动）

#### 2026-03-17 — 产品文档管理与产品知识驱动风险分析

**范围：**
- 后端 / 前端 / 文档 / 测试

**改动摘要：**
- **产品文档模块（新功能）**：新增 `/api/product-docs` 系列接口和 `/product-docs` 页面，支持导入 Markdown 产品文档、按 `##/###` 自动分块、在线编辑段落、查看并审核文档更新建议
- **项目绑定产品知识**：`Project` 新增 `product_code` 字段，项目可直接绑定对应产品文档；项目列表和编辑弹窗同步支持选择/展示所属产品
- **风险分析接入产品背景**：风险分析会在项目绑定产品文档时召回相关分块作为产品上下文，新增 `product_knowledge` 风险分类与 `risk_source` 来源标记；已接受风险支持保存产品澄清说明，并通过 `doc_update_needed` 标记是否需要更新产品文档
- **测试与规划文档补齐**：修正 `test_risk_service.py` 中对风险分析 LLM stub 的旧签名适配问题；补充规则树异步生成设计文档与实施计划文档

**涉及文件：**
- `backend/app/api/product_docs.py` (新增)
- `backend/app/schemas/product_doc.py` (新增)
- `backend/app/services/product_doc_service.py` (新增)
- `backend/app/api/projects.py`
- `backend/app/api/risks.py`
- `backend/app/core/schema_migrations.py`
- `backend/app/main.py`
...
- `backend/tests/test_risk_service.py`
- `frontend/src/App.tsx`
- `frontend/src/api/productDocs.ts` (新增)
- `frontend/src/api/projects.ts`
- `frontend/src/api/risks.ts`
- `frontend/src/pages/ProductDocs/index.tsx` (新增)
- `frontend/src/pages/ProjectList/index.tsx`
- `frontend/src/pages/RuleTree/RiskPanel.tsx`
- `frontend/src/types/index.ts`
- `docs/plans/2026-03-11-rule-tree-async-generation-design.md` (新增)
- `docs/plans/2026-03-11-rule-tree-async-generation.md` (新增)
- `KNOWLEDGE_BASE.md`

**接口/模型变更：**
- API: `POST/GET/DELETE /api/product-docs*` 与 `PUT /api/product-docs/*` 系列接口（新增）
- API: `PUT /api/rules/risks/{risk_id}/clarify` (新增)
- API: `POST /api/projects` / `PUT /api/projects/{project_id}` 请求体新增 `product_code?`
- Model: `Project` 新增 `product_code`
- Model: `RiskItem` 新增 `risk_source` / `clarification_text` / `doc_update_needed`
- Model: 新增 `ProductDoc` / `ProductDocChunk` / `ProductDocUpdate`

**验证记录：**
- `命令`: `cd backend && pytest -q`
- `结果`: 通过（`78 passed, 26 warnings`）
- `命令`: `cd frontend && npm run build`
- `结果`: 通过（保留既有第三方 `'use client'` 指令忽略与 chunk size 警告，不影响本次功能）

**兼容性说明：**
- 数据库变更均为新增表/新增列，继续通过启动时的 schema migration helper 自动补齐
- 未绑定 `product_code` 的既有项目行为不变，风险分析仍按原有规则树模式运行
- 产品文档更新建议接口已就绪；当前前端主流程已覆盖文档导入、编辑和审核，建议生成能力更适合与风险澄清或外部调用串联
...
**Q: 点击“接受风险”后，自动创建节点和手动“转节点”有什么区别？**
A: 两者底层都复用风险转节点逻辑，但触发时机不同。面板头部的 `接受时自动创建节点` 开关默认开启，因此点击「接受」会在保存决策后自动尝试转节点，并立即重算规则路径；手动「转节点」则适合后续补做或只想单独执行结构落库时使用。

**Q: 风险项可以删除吗？**
A: 可以。每条风险项右侧有「删除」按钮，点击后会弹出二次确认（Popconfirm），确认后物理删除该风险记录。删除操作不可恢复。

**Q: 勾选“需要更新产品文档”后，系统会自动生成文档更新建议吗？**
A: 会先保存风险澄清，然后前端会根据当前选中项目的 `product_code` 查找对应产品文档并调用 `POST /api/product-docs/suggest-update` 生成待审核建议。如果项目未绑定产品文档，或未找到匹配文档，则只提示警告，不影响澄清本身保存。
