...
22. 每条风险可操作：接受 / 忽略 / 转为规则节点 / 删除；已接受风险还可补充“产品澄清”并标记“需更新文档”
23. 删除操作通过 Popconfirm 二次确认后执行，物理删除该风险记录
24. 风险高亮：存在风险的节点在思维导图中以特殊标记显示（通过 `dataAdapter` 中的 `nodeRiskMap` 实现）
25. 存在待处理（pending）风险项时，「分析」按钮禁用并提示"请先处理所有待处理风险项"
26. 风险分析完成或决策处理后，通过 `onRisksChange` 回调同步更新父组件的风险状态
