...
│   │   │   ├── coverage.py         # 覆盖矩阵接口
│   │   │   ├── effective_requirements.py # 有效需求快照 / 分阶段风险收敛接口
...
│   │   │   ├── effectiveRequirements.ts # 有效需求 / 分阶段风险收敛 API
...
| requirement_id | Integer | FK → requirements.id, NOT NULL | 所属需求 |
| input_type | Enum | NOT NULL | 输入类型（`raw_requirement / clarification / product_decision / test_case_feedback`） |
...
| value | Text | 可选 | 字段值 |
| derivation | Enum | 可选 | 推导方式（原文 / 澄清 / 证据融合） |
...
| POST | `/api/requirements/{requirement_id}/snapshots/review` | 生成评审阶段有效需求快照 | — | ReviewSnapshotResponse |
...
| POST | `/api/ai/risks/predev-analyze` | 开发前风险收敛分析 | `{requirement_id}` | PredevAnalysisResponse |
| POST | `/api/ai/risks/prerelease-audit` | 提测前风险审计 | `{requirement_id}` | PrereleaseAuditResponse |
...
**核心流程：**
1. 收集需求文本和规则树全量节点信息
2. 若需求所属项目绑定了 `product_code`，则从对应产品文档中按关键词召回相关分块，拼接为 `product_context`
3. 通过 LLM 分析输出风险项列表（含风险描述、严重程度、建议处理方式、风险来源）
4. 服务层按 `requirement_id` 做并发合并：同一需求已有分析进行中时，后续重复请求阻塞等待，不再重复触发第二次 LLM 调用
5. 每轮分析落库前先删除该需求已有 `RiskItem`，再批量写入新风险项，保证风险列表只保留最新一轮结果
6. 用户逐条决策：接受（标注为已识别风险）、忽略（跳过该风险）、转节点（将风险转化为新的规则节点加入树中）、删除（物理删除该风险项）
7. 对已接受风险可补充产品澄清说明（`clarification_text`），并标记是否需要更新产品文档（`doc_update_needed`）
8. "转节点"操作会创建新的 `RuleNode` 并自动重算路径
9. "删除"操作通过前端 Popconfirm 二次确认后执行，物理删除风险记录
...
20. AI 分析当前规则树识别潜在风险点；若需求所属项目已绑定产品文档，分析时会一并利用产品背景知识识别 `product_knowledge` 风险
21. 风险列表按来源分组展示：`rule_tree`（技术风险）与 `product_knowledge`（产品知识风险）
22. 面板头部提供全局 `接受时自动创建节点` 开关，默认开启；点击「接受」时会将 `auto_create_node=true` 一并提交到风险决策接口
23. 当接受风险且自动建节点开关开启时，后端会立即把该风险转成异常节点并重算规则路径；前端在决策成功后通过 `onRiskConverted` 触发规则树刷新
24. 每条风险可操作：接受 / 忽略 / 转为规则节点 / 删除；已接受风险还可补充“产品澄清”并标记“需更新文档”
25. 在“产品澄清”弹窗中勾选“需要更新产品文档”后，前端会先调用 `PUT /api/rules/risks/{risk_id}/clarify` 保存澄清，再根据当前选中项目的 `product_code` 查找匹配的产品文档并调用 `POST /api/product-docs/suggest-update`
26. 若当前项目未绑定产品文档，或未找到匹配 `product_code` 的文档，只提示 warning，不影响本次澄清保存
27. 删除操作通过 Popconfirm 二次确认后执行，物理删除该风险记录
28. 风险高亮：存在风险的节点在思维导图中以特殊标记显示（通过 `dataAdapter` 中的 `nodeRiskMap` 实现）
29. 存在待处理（pending）风险项时，「分析」按钮禁用并提示"请先处理所有待处理风险项"
30. 风险分析完成或决策处理后，通过 `onRisksChange` 回调同步更新父组件的风险状态
