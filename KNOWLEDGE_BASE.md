...
- 若需求已明确选择 `matched_chains`，评审阶段会直接按链路感知召回产品知识，不再额外执行模块分析
- 评审阶段的产品知识召回查询同时包含需求原文与正式补充输入，避免补充说明仅进入 prompt、不参与召回
...
**核心流程：**
1. `risk_service.py` 负责原始风险识别：收集需求文本、规则树节点、产品知识上下文，并调用 LLM 产出风险项
...
3. `get_relevant_chunks` 根据需求文本或风险描述与分块关键词重叠度进行召回，优先保留“术语 / 总览 / 背景”等基础段落
4. `suggest_doc_update` 基于风险描述、人工澄清说明和原始段落生成一条 `ProductDocUpdate`
5. `apply_doc_update` 将建议内容写回原始分块，刷新关键词并同步提升文档版本号
6. `reject_doc_update` 仅更新建议状态，不修改原始文档内容
