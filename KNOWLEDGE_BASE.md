...
| **P1+** | 需求拆解 | 支持仅流程图、仅文字或图文联合分析，生成判断树/测试方案/风险点/用例矩阵并可一键导入 |
...
```json
{
  ...
  "test_cases": [
    {"title": "提现流程需求拆解-路径用例1", "steps": "依次验证: ...", "expected_result": "...", "risk_level": "high", "related_node_ids": ["dt_1", "dt_2"]}
  ]
}
```
...
- `llm` 模式下使用 SSE（`httpx + text/event-stream`）调用智谱模型；若返回结构存在常见“字段缺失/类型漂移”，会先做归一化修正再校验，仍失败才回退 `MockAnalyzerProvider`
...
3. **结果校验与兜底**：
   ...
   - 若校验失败，先尝试“结构归一化”修正常见漂移（如 `sections` 非字符串数组、`steps` 为数组、`related_node_ids` 为字符串、`risk_points` 缺少 `id` 等）
   - 归一化支持包裹层与别名字段：如 `data/result/output` 包裹、`decisionTree/testPlan/riskPoints/testCases`、`nodeId/nodeType/risk/level/actions/expected` 等
...
6. **生成测试用例** (`_build_generated_cases`) — 四产物之四
   ...
   - 用例风险等级取路径中最高风险等级
   - 用例标题命名规则：优先使用「需求标题-路径用例N」，无需求标题时回退为「架构路径用例 N」
...
| **需求拆解** | `/architecture` | 上传流程图和/或填写描述进行 AI 分析，展示判断树/测试方案/风险点/用例矩阵四 Tab，并支持选择性导入 |
...
- `test_architecture_analyzer.py` — 需求拆解分析引擎单测 + 全链路 API 测试 (覆盖仅流程图分析、图文联合分析、分析→查看→导入→验证规则树和用例；含包裹层 + 别名字段归一化回归)
- `test_llm_client.py` — LLM SSE 通道单测（覆盖流式拼接、JSON 提取、双重 JSON 编码解析、HTTP 错误与参数透传）
...
A: 先看几类高频根因：
- 网络连通性问题（常见于受限沙箱或未配置代理）：`open.bigmodel.cn` 无法解析/访问，会触发 `ConnectError` 并降级
- LLM 返回结构漂移（字段缺失/类型不符）：例如 `sections` 不是字符串数组、`steps` 为数组、`risk_points` 缺必填字段、输出被 `data/result` 包裹或使用 `camelCase`/别名字段；当前版本会先尝试归一化修正，修正后仍失败才降级
